...
class CompoundReader::ReaderFileEntry : LUCENE_BASE {
public:
...
public:
    ...
    const char* getDirectoryType() const override { return CompoundReader::getClassName(); }
...
void CSIndexInput::readInternal(uint8_t* b, const int32_t len) {
    std::lock_guard wlock(((CompoundDirectory::FSIndexInput*)base)->_this_lock);
...
CompoundReader::CompoundReader(lucene::store::Directory* d, const char* name,
...
void CompoundReader::copyFile(const char* file, int64_t file_length, uint8_t* buffer,
...
const char* CompoundReader::getClassName() {
    return "CompoundReader";
}
const char* CompoundReader::getObjectName() const {
    return getClassName();
}
...
bool CompoundReader::fileExists(const char* name) const {
    return entries->exists((char*)name);
}

lucene::store::Directory* CompoundReader::getDirectory() {
    return dir;
}

int64_t CompoundReader::fileModified(const char* name) const {
    return dir->fileModified(name);
}

int64_t CompoundReader::fileLength(const char* name) const {
...
bool CompoundReader::openInput(const char* name,
...
bool CompoundReader::openInput(const char* name, lucene::store::IndexInput*& ret,
                                    CLuceneError& error, int32_t bufferSize) {
    if (stream == nullptr) {
...
bool CompoundReader::doDeleteFile(const char* /*name*/) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: CompoundReader::doDeleteFile");
}

void CompoundReader::renameFile(const char* /*from*/, const char* /*to*/) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: CompoundReader::renameFile");
}

void CompoundReader::touchFile(const char* /*name*/) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: CompoundReader::touchFile");
}

lucene::store::IndexOutput* CompoundReader::createOutput(const char* /*name*/) {
    _CLTHROWA(CL_ERR_UnsupportedOperation,
              "UnsupportedOperationException: CompoundReader::createOutput");
}

std::string CompoundReader::toString() const {
    return std::string("CompoundReader@") + this->directory + std::string("; file_name: ") +
           std::string(file_name);
}
