...
bool InvertedIndexReader::indexExists(const std::string& file_path) {
    return _fs->path_exists(file_path).ok();
}

std::vector<std::wstring> InvertedIndexReader::get_analyse_result(const std::string& field_name,
    ...
    // auto analyser_type = InvertedIndexParserType::PARSER_STANDARD;
    // if (analyser_type == InvertedIndexParserType::PARSER_STANDARD) {
        ...
        reader = std::make_unique<lucene::util::StringReader>(std::wstring(value.begin(), value.end()).c_str());
    ...
    // } else {
    //     // default
    //     analyzer = std::make_unique<lucene::analysis::SimpleAnalyzer<TCHAR>>();
    //     reader = std::make_unique<lucene::util::StringReader>(std::wstring(value.begin(), value.end()).c_str());
    // }
    ...
    if (query_type == InvertedIndexQueryType::MATCH_ANY_QUERY ||
        ...
        std::sort(analyse_result.begin(), analyse_result.end());
    }
...
Status InvertedIndexReader::read_null_bitmap(InvertedIndexQueryCacheHandle* cache_handle,
    ...
    try {
        ...
        std::filesystem::path index_dir = _path.parent_path();
        std::string index_file_name = 
            InvertedIndexDescriptor::get_index_file_name(_path.filename(), index_id);
        InvertedIndexQueryCache::CacheKey cache_key {
                index_file_name, "", InvertedIndexQueryType::UNKNOWN_QUERY, L"null_bitmap"};
        ...
        if (!dir) {
            dir = new CompoundReader(
                    CompoundDirectory::getDirectory(_fs, index_dir.c_str()),
        ...
        std::shared_ptr<Roaring> null_bitmap = std::make_shared<Roaring>();
        ...
        if (dir->fileExists(null_bitmap_file_name.c_str())) {
            ...
            *null_bitmap = Roaring::read(reinterpret_cast<char*>(buf.data()), false);
            null_bitmap->runOptimize();
            cache->insert(cache_key, std::move(null_bitmap), cache_handle);
        ...
        return Status::InternalError("Inverted index read null bitmap error occurred");
    }

    return Status::OK();
}

Status FullTextIndexReader::new_iterator(OlapReaderStatistics* stats, InvertedIndexIterator** iterator) {
...
Status FullTextIndexReader::query(OlapReaderStatistics* stats, const std::string& column_name,
    ...
    std::string search_str = reinterpret_cast<const vpack::StringRef*>(query_value)->toString();
    LOG(INFO) << column_name << " begin to search the fulltext index from clucene, query_str ["
              << search_str << "]";

    std::filesystem::path index_dir = _path.parent_path();
    std::string index_file_name =
            InvertedIndexDescriptor::get_index_file_name(_path.stem(), index_id);
    ...
    try {
        ...
        if (analyse_result.empty()) {
            ...
            return Status::Unknown("INVERTED_INDEX_NO_TERMS");
        }
        ...
        auto index_search = [&](bool& null_bitmap_already_read,
                                std::shared_ptr<Roaring>& term_match_bitmap,
            ...
            if (!indexExists(index_file_path)) {
                LOG(WARNING) << "inverted index path: " << index_file_path.string()
                             << " not exist.";
                return Status::NotFound("inverted path not found");
            }
            ...
            try {
                ...
                return Status::Unknown("INVERTED_INDEX_CLUCENE_ERROR");
            }
        ...
        if (query_type == InvertedIndexQueryType::MATCH_PHRASE_QUERY) {
            ...
            InvertedIndexQueryCacheHandle cache_handle;
            std::shared_ptr<Roaring> term_match_bitmap = nullptr;
            if (cache->lookup(cache_key, &cache_handle)) {
                ...
                term_match_bitmap = std::make_shared<Roaring>();
            ...
            for (auto token_ws : analyse_result) {
                ...
                default: {
                    ...
                    return Status::NotSupported("fulltext query only support match query");
                }
                }
            }
        }
...
InvertedIndexReaderType starrocks::FullTextIndexReader::type() {
...
Status StringTypeInvertedIndexReader::query(OlapReaderStatistics* stats,
    ...
    auto index_file_name =
            InvertedIndexDescriptor::get_index_file_name(path.filename(), index_id);
    auto index_file_path = index_dir / index_file_name;
    ...
    if (!indexExists(index_file_path)) {
        ...
        return Status::Unknown("INVERTED_INDEX_FILE_NOT_FOUND");
    }
    ...
    default:
        ...
        return Status::Unknown("ErrorCode::INVERTED_INDEX_NOT_SUPPORTED");
    }
    ...
    try {
        ...
        if (_is_range_query(query_type) && e.number() == CL_ERR_TooManyClauses) {
            ...
            return Status::Unknown("ErrorCode::INVERTED_INDEX_BYPASS");
        } else {
            LOG(WARNING) << "CLuceneError occured, error msg: " << e.what()
                         << "column name:" << column_name << " search_str:" << search_str;
            return Status::Unknown("INVERTED_INDEX_CLUCENE_ERROR");
    ...
    // add to cache
    std::shared_ptr<Roaring> term_match_bitmap =
            std::make_shared<Roaring>(result);
    term_match_bitmap->runOptimize();
    cache->insert(cache_key, term_match_bitmap, &cache_handle);
...
BkdIndexReader::BkdIndexReader(FileSystem* fs, const std::string& path)
        : InvertedIndexReader(fs, path), _compoundReader(nullptr) {
    std::filesystem::path io_path(_path);
    auto index_dir = io_path.parent_path();
    auto index_file_name = 
            InvertedIndexDescriptor::get_index_file_name(io_path.filename(), index_id);
    ...
    _compoundReader = new CompoundReader(
            CompoundReader::getDirectory(fs, index_dir.c_str()), index_file_name.c_str(),
            config::inverted_index_read_buffer_size);
}
...
//         return Status::Unknown("INVERTED_INDEX_NOT_SUPPORTED");
//     }
...
Status BkdIndexReader::query(OlapReaderStatistics* stats, const std::string& column_name,
    ...
    // // add to cache
    // Roaring* term_match_bitmap = new Roaring(*bit_map);
    // term_match_bitmap->runOptimize();
    // cache->insert(cache_key, term_match_bitmap, &cache_handle);
...
InvertedIndexVisitor::InvertedIndexVisitor(Roaring* h, InvertedIndexQueryType query_type,
                                           bool only_count)
...
void InvertedIndexVisitor::visit(std::vector<char>& doc_id, std::vector<uint8_t>& packed_value) {
    ...
    visit(Roaring::read(doc_id.data(), false));
}
...
void InvertedIndexVisitor::visit(Roaring&& r) {
    if (_only_count) {
...
Status InvertedIndexIterator::read_from_inverted_index(const std::string& column_name,
    ...
    if (!skip_try && _reader->type() == InvertedIndexReaderType::BKD) {
        ...
        if (hit_count > segment_num_rows * query_bkd_limit_percent / 100) {
            ...
            return Status::Unknown("INVERTED_INDEX_BYPASS");
        }
    }
