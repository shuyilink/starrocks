...
public:
    ...
    static IndexSearcherPtr build_index_searcher(FileSystem* fs,
                                                 const std::string& index_dir,
                                                 const std::string& file_name);

    InvertedIndexSearcherCache(size_t capacity, uint32_t num_shards);

    Status get_index_searcher(FileSystem* fs, const std::string& index_dir,
    ...
    Status insert(FileSystem* fs, const std::string& index_dir,
                  const std::string& file_name);
...
public:
    ...
    InvertedIndexCacheHandle(const InvertedIndexCacheHandle &) = delete;
    const InvertedIndexCacheHandle &operator=(const InvertedIndexCacheHandle &) = delete;
...
public:
    // cache key
    struct CacheKey {
        std::filesystem::path index_path;               // index file path
    ...
    void insert(const CacheKey& key, std::shared_ptr<Roaring> bitmap,
...
public:
    ...
    std::shared_ptr<Roaring> get_bitmap() const {
        if (!_cache) {
            return nullptr;
        }
    ...
    InvertedIndexQueryCacheHandle(const InvertedIndexQueryCacheHandle &) = delete;
    const InvertedIndexQueryCacheHandle &operator=(const InvertedIndexQueryCacheHandle &) = delete;
private:
    Cache* _cache = nullptr;
    Cache::Handle* _handle = nullptr;
